...
class EnemyHandler:
    ...
    def on_update(self, delta_time: float = 1 / 60):
        ...
        if time_to_spawn is not None:
            self.time_to_spawn = font.LetterList(str(round(time_to_spawn)),
                                                 self.game_window.left_view + 500,
                                                 self.game_window.bottom_view + 500)
        else:
            self.time_to_spawn = None
...
class EnemyStationHandler(EnemyHandler):

    def __init__(self, game_window, mission_handler,
                 basic_types: list = None, boss_types: list = None,
                 mission_data: dict = None, target_object=None):

        super().__init__(game_window, mission_handler,
                         basic_types, boss_types,
                         mission_data, target_object)


class EnemyClusterHandler(EnemyHandler):
...
class EnemyEndlessHandler(EnemyHandler):
